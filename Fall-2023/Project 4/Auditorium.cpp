...
void Auditorium::unreserveSeat(Seat s)
{
    Node *cur = first;
    for(int i = 0; i < s.getRow() - 1; i++)
    {
        cur = cur->getDown();
    }
    for(int i = 0; i < (int)s.getColumn() - 65; i++)
    {
        cur = cur->getNext();
    }
    cur->getSeat().setTicketType('.');
}

int Auditorium::getOpenSeats()
{
    Node *cur = first;
    Node *hold = first;
    int numOpen = 0;
